...
export interface InstallConfig {
  serverCommand: string; // absolute path to the installed dist/server.js, run as `node <serverCommand>` in pinned mode
  autoUpdate: boolean; // true -> `npx -y @dendiem/caliper@latest serve` (resolves latest on each launch); false -> pinned `node <serverCommand>`
